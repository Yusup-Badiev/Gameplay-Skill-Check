...
## Key on guard guarding the door
- There is a key that can be looted from the guard at the door and used to unlock the door.
...
## Dialog with guard
- The guard at the door has two possible dialog options. One sets the tavern fight flag and sets the guard’s faction to evil when that flag is captured by the story script.
...
- After the first time you talk to the guard a flag "spoke_to_guard" will be set. In the dialog the starting nodes check if this flag was set and the guard will greet you with a different node

## guard Door Guard stats
